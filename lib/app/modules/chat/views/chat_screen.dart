...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _carregarDados() async {
    ...
    try {
      ...
      // 🔥 CASO 2: TEMOS PEDIDO_ID (fluxo do lojista)
      else if (widget.pedidoId != null) {
        // Obtém (ou cria) o chat a partir do pedido
        chatId = await _repository.getChatIdByPedido(widget.pedidoId!);
        // Busca o chat completo para exibir dados
        final chatData = await _repository.getChat(chatId);
        chatModel = chatData;
      }
      // 🔥 CASO 3: TEMOS LOJA_ID (fluxo de chat genérico, menos usado pelo lojista)
      else if (widget.lojaId != null) {
        final result = await _repository.iniciarChatComLoja(
          lojaId: widget.lojaId,
          pedidoId: null,
          mensagemInicial: null,
        );
        chatModel = result['chat'] as ChatModel;
        chatId = chatModel.id;
      } else {
        throw Exception('Não foi possível identificar o chat.');
      }
  ...
  // ================================================================
  // 🔥 AUTO REFRESH (SILENCIOSO) - usando metodo do lojista
  // ================================================================
  ...
  void _startAutoRefresh() {
    ...
    _refreshTimer = Timer.periodic(const Duration(seconds: 15), (timer) async {
      if (mounted && _chat != null && !_isSending) {
        try {
          final mensagens = await _repository.getMensagensLojista(_chat!.id);
  ...
  // ================================================================
  // 🔥 ENVIAR MENSAGEM (COM SKELETON) - usando metodo do lojista
  // ================================================================
  ...
  void _sendMessage() async {
    ...
    try {
      // 1️⃣ ENVIA MENSAGEM (metodo do lojista)
      await _repository.enviarMensagemLojista(
        _chat!.id,
        {
          'mensagem': text,
          'tipo': 'texto',
      ...
      // 2️⃣ RECARREGA TODAS AS MENSAGENS (metodo do lojista)
      final mensagens = await _repository.getMensagensLojista(_chat!.id);
    ...
    return Container(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final isMe = message.isDoCliente; // Lojista vê mensagens do cliente como "dele"?
          // 🔥 IMPORTANTE: Para o lojista, as mensagens enviadas por ele são "isMe = true".
          // O campo 'enviado_por' define isso. Vamos usar a lógica correta.
          final isFromLojista = message.enviadoPor == 'lojista';
          return ChatMessageBubble(
            message: message,
            isMe: isFromLojista,
            lojaNome: _chat?.lojaNome, // 🔥🔥🔥 PASSA O NOME DA LOJA AQUI!,
          );
        },
      ),
    );
